...
export async function POST(req: Request) {
  try {
    ...
    const plan = cleanStr(body?.plan);
    const amount = Number(body?.amount);
    const pin = cleanStr(body?.pin);

    if (!network || !phone || !plan || !Number.isFinite(amount) || amount <= 0 || !pin) {
      return json(400, { ok: false, message: 'Missing/invalid: network, phone, plan, amount, pin' });
    }
    ...
    const { data: txId, error: rpcErr } = await supabaseAdmin.rpc('purchase_data', {
      p_user_id: userId,
      p_amount: amount,
      p_currency: currency,
      p_network: network,
    ...
    return json(200, { ok: true, txId, network, phone, plan, amount, currency });
  } catch (e: any) {
