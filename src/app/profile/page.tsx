...
function useSupabaseUserRow() {
  ...
  const refresh = async (uid?: string | null) => {
    ...
    const { data, error } = await supabase
      .from('users')
      .select('id,email,full_name,phone_number,country,currency,profile_completed,status,invite_code')
      .eq('id', id)
      .maybeSingle();
  ...
  return useMemo(() => ({ sessionUserId, row, loading, refresh }), [sessionUserId, row, loading]);
}

const DashboardHeader = () => {
  const router = useRouter();
  const { row } = useSupabaseUserRow();
...
const ProfilePage: NextPage = () => {
  ...
  const { sessionUserId, row, loading, refresh } = useSupabaseUserRow();
  ...
  const inviteLink = useMemo(() => {
    ...
    return `${origin}/register?ref=${row.invite_code}`;
  }, [row?.invite_code, origin]);

  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: { fullName: '', phoneNumber: '', email: '', country: '', currency: '' },
    mode: 'onChange',
  });
                  ...
                  {loading ? (
                    ...
                    <Form {...form}>
                      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        {row?.invite_code && inviteLink && (
                          <div className="mt-4 space-y-2 rounded-lg border p-4 bg-muted/30">
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    toast({
                                      variant: 'destructive',
                                      title: 'Copy failed',
                                      description: 'Could not copy. Please hold and copy manually.',
                                    });
                                  }
                                }}
                              >
                            ...
                            <div className="text-xs text-muted-foreground">
                              Share this link. New users who register with it will be attached to your invite code.
                            </div>

                            <div className="text-sm font-medium pt-2">Your Invite Code</div>
                            <div className="flex items-center gap-2">
                              <Input readOnly value={row.invite_code} />
                              <Button
                                type="button"
                                variant="outline"
                                onClick={async () => {
                                  try {
                                    await navigator.clipboard.writeText(row.invite_code as string);
